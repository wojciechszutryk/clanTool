...
import { StyledRowHeader, StyledTitle, StyledClockNoisesWrapper, StyledSingleRow } from './styles';
...
const ClockNoises = ({ data }: Props) => {
  ...
  data.forEach((chartData, key) => {
    ...
    const deltaY = (Math.max(...yValuesArr) - Math.min(...yValuesArr)) / xyValueArrayLengthMinusOne;
    const clockNoiseSquares = chartData.map((_, index) => {
      if (index === chartData.length - 1) return null;
      const aParameter = (chartData[index + 1].y - chartData[index].y) / deltaY;
      return (
        <ClockNoiseSquare
          key={index}
