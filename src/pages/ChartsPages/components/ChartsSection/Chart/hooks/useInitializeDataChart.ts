...
const useInitializeDataChart = (
  ...
  useEffect(() => {
    ...
    if (xType === 'Date') chart.getDefaultAxisX().setTickStrategy(AxisTickStrategies.DateTime);
    ...
    chart
      .getDefaultAxisY()
      .setTickStrategy(AxisTickStrategies.Numeric, (numericTicks) => numericTicks);
    ...
    const series = chart
      ...
      .setCursorResultTableFormatter((builder, _, xValue, yValue) => {
        return builder
          .addRow(
            xType === 'Date' ? xType + ': ' : 'τ: ',
            xType === 'Date' ? new Date(xValue).toLocaleString() : xValue.toFixed(2).toString(),
          )
          .addRow(id + ': ', (yValue / CHART_ZOOM_FIX).toExponential(7).toString());
      });
