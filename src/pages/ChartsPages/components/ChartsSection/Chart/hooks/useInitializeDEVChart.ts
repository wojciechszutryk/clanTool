...
const useInitializeDEVChart = (
  ...
  useEffect(() => {
    ...
    const colors = [
      7, 24, 14, 19, 4, 9, 17, 18, 3, 22, 25, 5, 12, 16, 11, 10, 21, 23, 15, 13, 1, 8, 6, 2, 20,
    ].map(palette);
    ...
    const chart = lightningChart()
      ...
      .setAutoCursor((autoCursor) =>
        autoCursor.setResultTable((resultTable) =>
          resultTable.setTextFillStyle(new SolidFill({ color: ColorRGBA(0, 0, 0) })),
        ),
      )
    ...
    axisY.setTickStrategy(AxisTickStrategies.Numeric, (tickStrategy: NumericTickStrategy) =>
      tickStrategy
        .setMinorFormattingFunction((value, range) => {
          return (value / CHART_ZOOM_FIX).toExponential(3).toString();
        })
        .setMajorTickStyle((tickStyle) => tickStyle.setLabelFont((font) => font.setWeight('bold')))
        .setMajorFormattingFunction((value, range) => {
          return (value / CHART_ZOOM_FIX).toExponential(4).toString();
        }),
    );
    axisY.setTickStrategy(AxisTickStrategies.Numeric, (numericTicks) => numericTicks);

    const axisX = chart.getDefaultAxisX();
    axisX.setTickStrategy(AxisTickStrategies.Numeric, (tickStrategy: NumericTickStrategy) =>
      tickStrategy
        .setMinorFormattingFunction((value, range) => {
    ...
    data.forEach((chartPoints, devName) => {
      const pointSeries = chart
        ...
        .setCursorResultTableFormatter((builder, _, xValue, yValue) => {
          return builder
            .addRow('τ: ', (xValue * TAU_VALUE).toFixed(2).toString())
            .addRow(devName + ': ', (yValue / CHART_ZOOM_FIX).toExponential(7).toString());
        })
        .setName(devName)
