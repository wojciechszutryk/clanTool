...
const DatePicker = ({ isStartDate }: Props) => {
  ...
  return (
    <StyledDatePicker
      minDate={isStartDate ? dayjs(new Date(2014, 0, 1, 0, 0, 0, 0)) : dayjsDate(startDate)}
      maxDate={isStartDate ? dayjsDate(endDate) : dayjs(new Date(2021, 0, 1, 0, 0, 0, 0))}
      label={isStartDate ? 'Start date and Time' : 'End date and Time'}
      defaultValue={isStartDate ? dayjsDate(startDate) : dayjsDate(endDate)}
