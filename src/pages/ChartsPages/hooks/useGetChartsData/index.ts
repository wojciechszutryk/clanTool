...
const useGetChartsData = () => {
  ...
  const [workerState, setWorkerState] = useState<WorkerResMessage>({
    calculating: false,
    warning: undefined,
    error: undefined,
  ...
  const createChartsData = async (resourcesNames: string[], chartsToCreate: ChartTypes[]) => {
    ...
    calculateWorker.postMessage({ resourcesData, startDate, chartsToCreate, endDate, tauType, madMultiply });
    ...
    //       resourcesMap.set(getChartsDataMapKey(resourceName, ChartTypes.Phase), phaseChartData);
    //     }
    ...
    //       resourcesMap.set(getChartsDataMapKey(resourceName, ChartTypes.ADEV), allanDevChartData);
    //     }
